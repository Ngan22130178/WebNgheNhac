...
public class HomeController {
    ...
    // 3. Tìm kiếm bài hát: Xử lý khi người dùng nhập từ khóa vào ô tìm kiếm
    @GetMapping("/search")
    public String searchSongs(@RequestParam("keyword") String keyword, Model model) {
        // Gọi hàm tìm kiếm theo tên có chứa từ khóa đã viết ở Repository
        List<Song> searchResults = songRepository.findByTitleContainingIgnoreCase(keyword);
        
        // Gửi danh sách tìm được ra ngoài giao diện trang index
        model.addAttribute("songs", searchResults);
        return "fragments/songs_table"; // Trả về fragment chứa bảng bài hát để cập nhật nội dung bằng AJAX
    }
}